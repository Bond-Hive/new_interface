...
const MainDapp = () => {
  ...
  const getReserveContractCal = async (
    ...
    const tx = txBuilder
      .addOperation(contract.call("total_deposit"))
      .setTimeout(30)
      .build();
  ...
  const getPoolReserve = async (poolIndex: number) => {
    ...
    const poolReserve: any = await getReserveContractCal(
  ...
  const readContract = async (functName: string, index: number) => {
    ...
    const result: any = await readContIntr(
  ...
  const sortFunc = (type: string) => {
    setPools((prevPools: any) => {
      const sortedPools = [...prevPools].sort((a, b) => {
        const valueA =
          type === "apy" ? parseFloat(a.apy) : parseFloat(a.reserves);
        const valueB =
          type === "apy" ? parseFloat(b.apy) : parseFloat(b.reserves);
  ...
  const ETH_APY_DATA: number[] = [
    0, 2, 5, 8, 2, 16, 8, 30, 30, 35, 36, 30, 10, 37, 40, 45, 37, 40, 38, 35,
    25, 23, 28, 50, 48, 45, 43, 45, 38, 35, 54, 40, 60, 80, 78, 76, 74, 72, 70,
    68, 65, 63, 80, 90, 100, 150, 139, 170, 140, 180, 200,
  ];
  const BTC_APY_DATA: number[] = [
    0, 10, 30, 8, 20, 10, 20, 30, 30, 25, 36, 30, 25, 50, 40, 45, 37, 40, 38,
    35, 25, 23, 28, 50, 48, 45, 43, 45, 38, 90, 54, 40, 60, 80, 78, 76, 90, 72,
    70, 68, 65, 63, 80, 90, 140, 150, 139, 170, 140, 180, 200,
  ];
  ...
  return (
    <>
      <div className="dapp">
        ...
        <div className="md:w-9/12 md:max-lg:w-11/12 mx-auto md:pt-24 pt-8 px-5">
          {selectedNetwork.network === "TESTNET" && (
            <div className="card max-w-[1100px] mx-auto px-4 text-lg text-center py-9 mb-6">
              <p className="text-white text-2xl">
                You are connected to the wrong network. Please switch to
                mainnet.
              </p>
              <Link href={"https://testnet.bondhive.xyz/app"} target="_blank">
                <p className=" text-white underline">Link to use testnet</p>
              </Link>
            </div>
          )}
          {/* three cards */}
              ...
              <div className="eth_avg max-sm:w-6/12 max-sm:h-[184px] w-6/12 h-[136px] card relative overflow-hidden">
                <div className="flex justify-between items-center md:p-4 p-2">
                  <div className="">
                    ...
                    <h1 className="text-[20px] text-white mt-2 brFirma_font">
                      13.61%
                    </h1>
                  </div>
                </div>
                  ...
                  {activePools.map((pool: any, index: number) => (
                    ...
                    >
                      <div className="strategy_Names w-4/12 bg-red-60">
                        <div className="flex items-center mb-3 gap-2">
                          ...
                          <div className="">
                            ...
                            <p className="text-darkPrimText text-sm mt-2">
                              Minimum{" "}
                      ...
                      <div className="flex flex-col gap-4 items-cente w-3/12">
                        {connectorWalletAddress &&
                          Number(pool?.shareBalance) > 0 && (
                            <button
                              className={`px-9 py-[7px] gap-1 cursor-pointer ${
                                !connectorWalletAddress ||
                                Number(pool?.shareBalance) <= 0
                            ...
                            >
                              {!pool?.shareBalance && connectorWalletAddress ? (
                        ...
                        <button
                          className={` flex items-center w-full py-[7px] gap-1 justify-center ${
                            !connectorWalletAddress ||
                            !pool.depositEnabled ||
                            selectedNetwork.network == "TESTNET"
                              ? "hover:bg-transparent button1"
                          ...
                          id={!pool.depositEnabled ? "depositDisabled" : ""}
                        >
                          <p className="text-sm ">
                            {!pool.depositEnabled ? "Maturity reached" : "Invest"}
                          </p>
                          <Image
                            src={chervonRight}
                            width={13}
                    ...
                    >
                      ...
                      <div className="text-[16px] py-4">
                        ...
                        <div className="min_invest flex justify-between items-center">
                          ...
                          <p className=" text-blueish">$100</p>
                        </div>
                      </div>
                      {connectorWalletAddress &&
                        Number(pool?.shareBalance) > 0 && (
                          <button
                            ...
                            disabled={
                              !connectorWalletAddress ||
                              Number(selectedPool.shareBalance) <= 0
                            }
                          >
                            {!shareBalance && connectorWalletAddress ? (
                              <div className="flex justify-center">
                                <Loading />
                              </div>
                            ) : (
                      ...
                      <button
                        className={`w-full mt-3 flex items-center justify-center px-9 py-3 gap-1 ${
                          !connectorWalletAddress || !pool.depositEnabled
                            ? "hover:bg-transparent button1"
                            : "button2"
                        }`}
                        onClick={() => {
                        ...
                        disabled={!connectorWalletAddress || !pool.depositEnabled}
                        id={!pool.depositEnabled ? "depositDisabled" : ""}
                      >
      ...
      {openWithdrawState && (
        <WithdrawFunds setOpenState={setOpenWithdrawState} />
      )}
      {/* {
      !pool.
